...
| --no-rest-swagger | 不添加REST Framework和Swagger配置 | False | --no-rest-swagger |
...
- 🔄 修改三个配置文件：
  ...
  - `config/settings/logging_config.py`：添加应用日志配置
  - `config/urls.py`：添加URL配置
- ⚡ 自动备份修改的文件到：
  - `config/app_append_backups/base_backups/`
  - `config/app_append_backups/logging_backups/`
